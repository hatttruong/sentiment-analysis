...
tf.flags.DEFINE_string(
    "positive_data_file",
    helper.get_processed_train_path(
        DATA_DIR + 'Positive_train.csv.preprocessed'),
    "Data source for the positive data.")
tf.flags.DEFINE_string(
    "negative_data_file",
    helper.get_processed_train_path(
        DATA_DIR + 'Negative_train.csv.preprocessed'),
    "Data source for the negative data.")
tf.flags.DEFINE_string(
    "neutral_data_file",
    helper.get_processed_train_path(
        DATA_DIR + 'Neutral_train.csv.preprocessed'),
...
def preprocess():
    ...
    x_text, y = helper.load_data_and_labels(
        FLAGS.positive_data_file,
        FLAGS.negative_data_file,
        FLAGS.neutral_data_file)

    # Build vocabulary
    max_document_length = 200  # max([len(x.split(" ")) for x in x_text])
    ...
    logger.info("Vocabulary Size: {:d}".format(
...
def train(x_train, y_train, vocab_processor, x_dev, y_dev):
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            logger.info('TextCNN: num_classes=%s', y_train.shape[1])
            ...
            def train_step(x_batch, y_batch):
                ...
                logger.info("\n{}: TRAIN step {}, loss {:g}, acc {:g}".format(
...
logging.basicConfig(
    filename='log/log_train_cnn.log',
